    ...
    public class BehaviourTreeView : GraphView
    {
        ...
        public void PopulateView(BehaviourTree tree)
        {
            ...
            tree.nodes.ForEach(n =>
            {
                ...
                children.ForEach(c =>
                {
                    //FindNodeView로 제대로된 child를 찾아오지 못함
        ...
        private NodeView FindNodeView(Node node)
        {
            ...
            Debug.Log($"GetNodeByGuild: {GetNodeByGuid(node.guid) as NodeView}");
        ...
        public override void BuildContextualMenu(ContextualMenuPopulateEvent evt)
        {
            ...
            foreach(Type brainType in BTBrain.typeList)
            {
                {
...

                {
                    ...
                    var compositeNodeBaseType = typeof(CompositeNode<>).MakeGenericType(brainType);
                    var derivedTypes = AppDomain.CurrentDomain.GetAssemblies()
                        .SelectMany(s => s.GetTypes())
                        .Where(p => compositeNodeBaseType.IsAssignableFrom(p) && p.IsClass && !p.IsAbstract);
                    foreach (var type in derivedTypes)
                    {
                        evt.menu.AppendAction($"[{type.BaseType.Name}]/{type.Name} ", (a) => CreateNode(type, nodePostion));
                    }
...

                {
                    ...
                    var decoratorNodeBaseType = typeof(DecoratorNode<>).MakeGenericType(brainType);
                    var derivedTypes = AppDomain.CurrentDomain.GetAssemblies()
                        .SelectMany(s => s.GetTypes())
                        .Where(p => decoratorNodeBaseType.IsAssignableFrom(p) && p.IsClass && !p.IsAbstract);
                    foreach (var type in derivedTypes)
                    {
                        evt.menu.AppendAction($"[{type.BaseType.Name}]/{type.Name} ", (a) => CreateNode(type, nodePostion));
                    }
